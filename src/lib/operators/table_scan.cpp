  ...
  resolve_data_type(type_name, [&](auto type) {
    ...
    for (ChunkID chunk_index = ChunkID(0); chunk_index < input_chunk_count; ++chunk_index) {
      ...
      if (const auto value_segment = std::dynamic_pointer_cast<ValueSegment<Type>>(segment)) {
        ...
        std::iota(input_filter.begin(), input_filter.end(), 0);
        _scan_dictionary_segment(pos_list, typed_search_value, chunk_index, dictionary_segment, input_filter);
...
void TableScan::_scan_reference_segment(const std::shared_ptr<PosList> pos_list,
                                        const std::function<bool(T, T)>& comparator, const T typed_search_value,
  ...
  for (auto position_iterator = reference_segment->pos_list()->begin();
       position_iterator < reference_segment->pos_list()->end();) {
    ...
    auto referenced_segment =
        reference_segment->referenced_table()->get_chunk(referenced_chunk_id).get_segment(_column_id);
    if (const auto value_segment = std::dynamic_pointer_cast<ValueSegment<T>>(referenced_segment)) {
      ...
      _scan_dictionary_segment(pos_list, typed_search_value, referenced_chunk_id, dictionary_segment, input_filter);
    }
  }
}
...
void TableScan::_scan_value_segment(const std::shared_ptr<PosList> pos_list,
                                    const std::function<bool(T, T)>& comparator, const T typed_search_value,
                                    const ChunkID& chunk_index, const std::shared_ptr<ValueSegment<T>> value_segment,
...
template <typename T>
void TableScan::_scan_dictionary_segment(const std::shared_ptr<PosList> pos_list, const T typed_search_value,
